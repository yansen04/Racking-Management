...
export function ItemPlacement() {
  ...
  const place = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/placement', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
    if (!res.ok) {
      const msg = await res.text();
      toast.error(msg || 'Failed to place item');
      return;
    }
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-semibold">Item Placement</h1>

      <form onSubmit={place} className="card p-4 space-y-3">
